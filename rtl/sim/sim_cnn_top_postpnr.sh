#!/bin/bash
# Post-PnR gate-level simulation (functional, no SDF) using Icarus Verilog
# Usage: ./sim_cnn_top_postpnr.sh [run_name] [num_images] [clk_period_ns] [dump_vcd]
...
RUN_NAME="${1:?Usage: $0 <run_name> [num_images] [clk_period_ns] [dump_vcd]}"
NUM_IMAGES="${2:-3}"
CLK_PERIOD="${3:-100}"
DUMP_VCD="${4:-0}"

RUN_DIR="${REPO_ROOT}/librelane_flow/cnn_top/runs/${RUN_NAME}"
NETLIST="${RUN_DIR}/final/nl/cnn_top.nl.v"

echo "=== Post-PnR GLS (functional, no SDF) with Icarus ==="
echo "  Run:       $RUN_NAME"
echo "  Netlist:   $NETLIST"
echo "  PDK:       $PDK_DIR"
echo "  Images:    $NUM_IMAGES"
echo "  Clock:     ${CLK_PERIOD} ns"
echo "  Dump VCD:  $DUMP_VCD"
echo ""
...
iverilog -g2012 -DPOSTSYNTH -DFUNCTIONAL -o ${SIM_DIR}/tb_top_spi_postpnr.out \
...
cd ${SIM_DIR} && vvp tb_top_spi_postpnr.out \
    +NUM_IMAGES=${NUM_IMAGES} +CLK_PERIOD_NS=${CLK_PERIOD} +DUMP_VCD=${DUMP_VCD}
